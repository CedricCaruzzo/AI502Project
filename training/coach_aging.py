...
class Coach:
	def __init__(self, opts):
		...
		pretrained_model_path = 'pretrained_models/iteration_900.pt'
		if self.opts.fine_tune:
			if os.path.exists(pretrained_model_path):
				...
				print('!!Cannot find the pretrained model file. Starting training from scratch!!!')
		else:
	...
	#####################MODIFICATION################################
	#This function is for x_input race vector
	def get_race_vector_input(self, data):
		race_dict = data[0]['race']
		dominant_race = data[0]['dominant_race']
		
		race_vector = [0] * len(race_dict)
		race_index = list(race_dict.keys()).index(dominant_race)
		race_vector[race_index] = 1
		
		return torch.from_numpy(np.array(race_vector)).to(self.device)
	
	#This function is for the y_hat race vector
	def get_race_vector_output(self, data):
		race_dict = data[0]['race']
		dominant_race = data[0]['dominant_race']
		
		race_values = [race_dict[race] / 100 for race in race_dict]
		
		return torch.from_numpy(np.array(race_values)).to(self.device)
	
	#This function take a single image and put it through the Deep Face Pipeline
	def get_race_from_image(self, image):
		inputs_race = DeepFace.analyze(image, actions=("race"), enforce_detection=False, detector_backend="opencv", align=True, silent=True)
		return inputs_race
	################################################################
	...
	def calc_loss(self, x, y, y_hat, latent, target_ages, input_ages, no_aging, data_type="real"):
		...
		if (self.opts.race_lambda > 0):
			inputs_race = [DeepFace.analyze((image + 1) * 127.5 , actions=("race"),
													...
													align=True, silent=True) for image in x.permute(0, 2, 3, 1).cpu().numpy()]

			outputs_race = [DeepFace.analyze((image + 1) * 127.5 , actions=("race"),
													enforce_detection=False, detector_backend="opencv",
													align=True, silent=True) for image in y_hat.permute(0, 2, 3, 1).cpu().detach().numpy()]

			inputs_race_logits = torch.from_numpy(np.array([self.get_race_vector_input(input_race).cpu() for input_race in inputs_race])).to(self.device)
			outputs_race_logits = torch.from_numpy(np.array([self.get_race_vector_output(output_race).cpu() for output_race in outputs_race])).to(self.device)
			
			crossEntropyLoss = nn.CrossEntropyLoss()
			inputs_race_indices = inputs_race_logits.argmax(dim=1)
			race_loss = crossEntropyLoss(outputs_race_logits, inputs_race_indices)
